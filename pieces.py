...
class PieceFactory:
    ...
    def make(id, pos):
        class_ = getattr(
            sys.modules[__name__], id.type.name.capitalize()
        )
        return class_(id.color, pos)
...
class Piece(pg.sprite.Sprite):
    def __init__(self, type, color, pos):
        ...
        self.pos = pos
        logging.info('Creating %s at %r', type.name, pos)
        self.rect.center = self.pos * s.TILESIZE * 2 + vec(s.TILESIZE, s.TILESIZE)
            ...
            for coords in diag:
                if grid[coords]:
                    ...
                    if Color(piece.color) != self.color:
                        moves.append(vec(coords[1], coords[0]))
                    break
    ...
    @property
    def gpos(self):
        return (int(self.pos.y), int(self.pos.x))
